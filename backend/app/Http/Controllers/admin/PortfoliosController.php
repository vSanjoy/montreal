...
# Author            :
# Created Date      :
# Page/Class name   : PortfoliosController
...
class PortfoliosController extends Controller
{
    ...
    */
    public function add(Request $request, $id = null) {
        ...
        try {
            $data['serviceList'] = serviceDropdownMenu();
            ...
            if ($request->isMethod('POST')) {
                ...
                $validationMessages = array(
                    ...
                    'service_ids.required'  => trans('custom_admin.error_service'),
                ...
                if ($validator->fails()) {
                    ...
                    if ($save) {
                        ...
                        if (isset($request->service_ids) && count($request->service_ids)) {
                            $portfolioServiceMapping = [];
                            foreach ($request->service_ids as $key => $item) {                    
                                $portfolioServiceMapping[$key]['portfolio_id']  = $save->id;
                                $portfolioServiceMapping[$key]['service_id']    = $item;
                            }
                            if (count($portfolioServiceMapping)) {
                                PortfolioServiceMapping::insert($portfolioServiceMapping);
                            }
                        }
    ...
    */
    public function edit(Request $request, $id = null) {
        ...
        try {
            ...
            if ($request->isMethod('POST')) {
                ...
                if ($validator->fails()) {
                    ...
                    if ($update) {
                        // Inserting services data
                        if (isset($request->service_ids) && count($request->service_ids)) {
                            PortfolioServiceMapping::where('portfolio_id', $id)->delete();
                            $portfolioServiceMapping = [];
                            foreach ($request->service_ids as $key => $item) {                    
                                $portfolioServiceMapping[$key]['portfolio_id']  = $details->id;
                                $portfolioServiceMapping[$key]['service_id']    = $item;
                            }
                            if (count($portfolioServiceMapping)) {
                                PortfolioServiceMapping::insert($portfolioServiceMapping);
            ...
            $portfolioServiceIds = [];
            if ($details->portfolioServiceMapping) {
                foreach ($details->portfolioServiceMapping as $item) {
                    $portfolioServiceIds[] = $item->service_id;
                }
            }
            $data['portfolioServiceIds'] = $portfolioServiceIds;
