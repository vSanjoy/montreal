...
$(document).ready(function() {
	...
	// Show modal with enquiry details
	$(document).on('click', '.click-to-open', function() {
		var id = $(this).data('id');
		if (id != '') {
			$('.preloader').show();
			var getPopupDataUrl = $('#admin_url').val()+'/enquiry/view/'+id;
			$.ajax({
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
				},
				...
				success: function (response) {
					...
					if (response.type == 'success') {
						$('#enquiry-data').html(response.message);
					} else {
						$('#enquiry-data').html('<tr><td colspan="2">{{trans("custom_admin.message_no_records_found")}}</td></tr>');
					}
				}
			});
		} else {
