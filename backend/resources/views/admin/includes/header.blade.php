@php $enquiryList = getEnquiryList(); @endphp
            ...
            <div class="navbar-brand">
                ...
                <a href="{{ url('/adminpanel') }}">
                    ...
                    <span class="logo-text">
                        <!-- dark Logo text -->
                        <img src="{{asset('images/admin/logo-text.png')}}" alt="" class="dark-logo" />
                        <!-- Light Logo text -->
        ...
        <div class="navbar-collapse collapse" id="navbarSupportedContent">
            ...
            <ul class="navbar-nav float-left mr-auto ml-3 pl-1">
                ...
                <li class="nav-item dropdown">
                    ...
                    @if ($enquiryList->count())
                        <ul class="list-style-none">
                            <li>
                                <div class="message-center notifications position-relative">
                                @foreach ($enquiryList as $item)
                                    <a class="message-item d-flex align-items-center border-bottom px-3 py-2">
                                        <span class="btn btn-success rounded-circle btn-circle"><i data-feather="mail" class="text-white" title=""></i></span>
                                        <div class="w-75 d-inline-block v-middle pl-2">
                                            <h6 class="message-title mb-0 mt-1">{{ ucwords($item->name) }}</h6>
                                            <span class="font-12 text-nowrap d-block text-muted">{{ $item->email }}</span>
                                            <span class="font-12 text-nowrap d-block text-muted text-truncate">{!! excerpts($item->message, 8) !!}</span>
                                            <span class="font-12 text-nowrap d-block text-muted">{{ changeDateFormat($item->created_at) }}</span>
                                        </div>
                                    </a>
                                @endforeach
                                </div>
                            ...
                            <li>
                                <a class="nav-link pt-3 text-center text-dark" href="{{ route('admin.enquiry.list') }}">
                                    <strong>@lang('custom_admin.label_see_all_enquiries')</strong>
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="http://demo4workz.com/montreal-designs/" target="_blank">
