		...
		if (input.files[0].size > {{config('global.MAX_UPLOAD_IMAGE_SIZE')}}) {
			...
			if (extension == 'jpeg' || extension == 'jpg' || extension == 'gif' || extension == 'png' || extension == 'bmp') {
				...
				reader.onload = function (e) {
					...
					$('#'+imagePreviewId+'_preview').after('<div class="image-preview-holder" id="image_holder_'+imagePreviewId+'"><img src="'+e.target.result+'" class="image-preview-border" width="90" /><span class="delete-preview-image" data-cid="'+imagePreviewId+'"><i class="fa fa-trash"></i></span></div>');
				};
